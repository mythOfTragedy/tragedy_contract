...
async function main() {
    console.log("📊 Generating Token CSV locally (without blockchain calls)...\n");
    ...
    // Function to get display effect
    ...
    // CSV header
    const csvRows = [
        "TokenID,Species,Equipment,Realm,Curse,Name,Story,Rarity,Synergy,SpeciesID,ItemID,BackgroundID,EffectID"
    ];
    ...
    for (let tokenId = 1; tokenId <= 10000; tokenId++) {
        ...
        // Add row
        csvRows.push(
            `${tokenId},${monsterName},${displayItemName},${backgroundName},${effectName},"${name}","${story}",${rarity},"${synergy.title}",${species},${item},${background},${displayEffect.id}`
        );
    ...
    const filename = `token-mapping-${Date.now()}.csv`;
    ...
    // Show special tokens
    console.log("\n🌟 Special Tokens:");
    console.log("  Token #1230: Toxic Abomination");
    console.log("  Token #1961: Soul Harvester");
}
