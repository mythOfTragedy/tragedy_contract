...
async function main() {
    console.log("📊 Generating Token CSV with Seed 7001 and new effect mapping...\n");
    ...
    const filename = `token-mapping-seed7001-${Date.now()}.csv`;
    ...
    // Show special tokens
    console.log("\n🌟 Special Tokens:");
    console.log("  Token #437: Toxic Abomination (Frankenstein + Poison + Venom + Matrix)");
    console.log("  Token #2962: Soul Harvester (Skeleton + Scythe + Shadow + Blackout)");
    
    // Show some legendary IDs
    console.log("\n🏆 Some Legendary Token IDs:");
    const legendaryIds = [1, 7, 13, 23, 42];
    legendaryIds.forEach(id => {
        const row = csvRows[id];
        if (row) {
            const parts = row.split(',');
            console.log(`  Token #${id}: ${parts[1]} with ${parts[2]} (${parts[7]})`);
        }
    });
}
